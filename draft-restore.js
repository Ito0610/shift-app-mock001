...
(function () {
  ...
  function restoreDraft() {
    ...
    if (gasUrl && gasUrl.indexOf('script.google.com') !== -1 && employeeName) {
      var query = '?draft=1&employeeName=' + encodeURIComponent(employeeName) + '&year=' + year + '&month=' + month;
      fetch(gasUrl + query)
        .then(function (res) { return res.json(); })
        .then(function (data) {
          if (data && (data.monthNotes != null || (data.days && Object.keys(data.days).length > 0))) {
